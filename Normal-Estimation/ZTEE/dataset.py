...
class PointcloudPatchDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        patch_pts = torch.zeros(self.points_per_patch*len(self.patch_radius_absolute[shape_ind]), 3, dtype=torch.float) #128*3

        if self.points_per_patch==700:
            neighbor_normals = torch.zeros(175* len(self.patch_radius_absolute[shape_ind]), 3,
                                dtype=torch.float) #128*3
        else:
        ...
        for s, rad in enumerate(self.patch_radius_absolute[shape_ind]):
            if self.neighbor_search_method == 'r':
                ...
                point_distances, patch_point_inds = shape.kdtree.query(shape.pts[center_point_ind, :], k=self.points_per_patch)# (128,) (128,)
        ...
        if self.use_pca:
            ...
            trans, _, _ = torch.svd(torch.t(patch_pts[patch_pts_valid, :]))# 3*3
